...
/**
 * Create a KindOfOrganization and make a Thing a member_of_kind of it, then
 * persist the Thing and make sure it is read back as an Organization.
 *
 * <p>
 * This demonstrates the implementation of instances of 'kinds' as new entity
 * types.
 * </p>
 */
public class CreateOrganizationSubTypeUsingKinds {
   ...
   */
  private static IriBase TEST_BASE =
      new IriBase("test", "http://example.com/test#");
  ...
  public void test() {
    ...
    final KindOfOrganizationComponent componentKind =
        new KindOfOrganizationComponentBuilder(randomIri()).build();

    // Create a new kind of organization to represent companies.
    // This will also make the new kind into a sub-type of organization.
    final KindOfOrganization companies =
        new KindOfOrganizationBuilder(randomIri())
            .has_Component_By_Class_M(componentKind)
            .build();

    // Create an Individual and make it a member_of_kind of the companies kind.
    // This will also mean that the individual is of type organization, via its
    // kind.
    ...
    // Persist everything.
    final MagmaCoreService service =
        MagmaCoreServiceFactory.createWithJenaDatabase();
    service.beginWrite();
    ...
    // Confirm that the entity created as an individual is now actually an
