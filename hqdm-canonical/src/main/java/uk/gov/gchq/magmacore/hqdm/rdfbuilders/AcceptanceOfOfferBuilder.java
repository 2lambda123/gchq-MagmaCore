...
public class AcceptanceOfOfferBuilder {
  ...
  public AcceptanceOfOfferBuilder(final IRI iri) {
    this.acceptanceOfOffer =
        SpatioTemporalExtentServices.createAcceptanceOfOffer(iri);
  }
   ...
   * @return This builder.
   */
  public final AcceptanceOfOfferBuilder
  aggregated_Into(final SpatioTemporalExtent spatioTemporalExtent) {
    this.acceptanceOfOffer.addValue(AGGREGATED_INTO,
  ...
  consists_Of_Participant(final Participant participant) {
    this.acceptanceOfOffer.addValue(CONSISTS_OF_PARTICIPANT,
                                    participant.getId());
    return this;
  }
  ...
  public final AcceptanceOfOfferBuilder member_Of(
      final ClassOfSociallyConstructedActivity
          classOfSociallyConstructedActivity) {
    this.acceptanceOfOffer.addValue(MEMBER_OF,
   ...
   * A {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#PART_OF} relationship type
   * where an
   * {@link AcceptanceOfOffer} is {@link
   * uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#PART_OF} just one
   * {@link AgreeContract}.
   *
   ...
   */
  public final AcceptanceOfOfferBuilder
  part_Of_M(final AgreeContract agreeContract) {
  ...
  part_Of_Possible_World_M(final PossibleWorld possibleWorld) {
    this.acceptanceOfOffer.addValue(PART_OF_POSSIBLE_WORLD,
                                    possibleWorld.getId());
    return this;
  }

  /**
   * A {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#REFERENCES} relationship
   * type where an
   * {@link AcceptanceOfOffer} {@link
   * uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#REFERENCES} exactly one
  ...
  temporal__Part_Of(final SpatioTemporalExtent spatioTemporalExtent) {
    this.acceptanceOfOffer.addValue(TEMPORAL__PART_OF,
                                    spatioTemporalExtent.getId());
    return this;
  }
  ...
  /**
   * Returns an instance of AcceptanceOfOffer created from the properties set on
   * this builder.
   *
   * @return The built AcceptanceOfOffer.
   * @throws HqdmException If the AcceptanceOfOffer is missing any mandatory
   *     properties.
   */
