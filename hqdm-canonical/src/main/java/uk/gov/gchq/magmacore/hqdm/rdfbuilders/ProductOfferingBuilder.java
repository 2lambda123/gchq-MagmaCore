...
public class ProductOfferingBuilder {
  ...
  /**
   * A {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#CLASS_OF_OFFERED}
   * relationship type where a
   * {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#MEMBER_OF} a {@link
   * SalesProduct} is offered.
   *
   * @param salesProduct The SalesProduct.
   * @return This builder.
   */
  public final ProductOfferingBuilder
  class_Of_Offered_M(final SalesProduct salesProduct) {
   ...
   * uk.gov.gchq.magmacore.hqdm.model.Offering} has exactly one price at which
   * the {@link uk.gov.gchq.magmacore.hqdm.model.Offering} is made.
   *
   * @param price The Price.
   * @return This builder.
   */
  public final ProductOfferingBuilder
  consideration_By_Class_M(final Price price) {
  ...
  public final ProductOfferingBuilder member_Of_(
      final ClassOfClassOfSpatioTemporalExtent
          classOfClassOfSpatioTemporalExtent) {
    this.productOffering.addValue(MEMBER_OF_,
  ...
  /**
   * A relationship type where an {@link
   * uk.gov.gchq.magmacore.hqdm.model.Offering} has exactly one
   * {@link Party} who makes the {@link
   * uk.gov.gchq.magmacore.hqdm.model.Offering}.
   *
  ...
  public final ProductOfferingBuilder part__Of_By_Class(
      final ClassOfSpatioTemporalExtent classOfSpatioTemporalExtent) {
    this.productOffering.addValue(PART__OF_BY_CLASS,
                                  classOfSpatioTemporalExtent.getId());
    return this;
  }

  /**
   * A {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#PART_OF_BY_CLASS}
   * relationship type where a
   * {@link uk.gov.gchq.magmacore.hqdm.rdf.iri.HQDM#MEMBER_OF} a
   * {@link uk.gov.gchq.magmacore.hqdm.model.ClassOfSociallyConstructedActivity}
   * may be a
   ...
   */
  public final ProductOfferingBuilder
  part_Of_By_Class(final ClassOfReachingAgreement classOfReachingAgreement) {
    this.productOffering.addValue(PART_OF_BY_CLASS,
   ...
   */
  public final ProductOfferingBuilder
  part_Of_By_Class_(final ClassOfAgreementExecution classOfAgreementExecution) {
    this.productOffering.addValue(PART_OF_BY_CLASS_,
   ...
   */
  public final ProductOfferingBuilder
  period_Offered_M(final PeriodOfTime periodOfTime) {
    this.productOffering.addValue(PERIOD_OFFERED, periodOfTime.getId());
    return this;
  }

  /**
   * Returns an instance of ProductOffering created from the properties set on
   * this builder.
   *
   * @return The built ProductOffering.
   * @throws HqdmException If the ProductOffering is missing any mandatory
   *     properties.
   */
